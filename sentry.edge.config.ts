...
if (process.env.NODE_ENV === "production" && !process.env.NEXT_PUBLIC_SENTRY_DSN) {
  console.error("[sentry:edge] NEXT_PUBLIC_SENTRY_DSN is not set — error reporting is DISABLED.");
}
...
Sentry.init({
  ...
  // Scrub sensitive data — edge runs middleware that handles JWTs and cookies.
  // Without beforeSend, tokens/cookies leak into Sentry error payloads.
  ...
  beforeSend(event) {
    ...
    // Redact request body (may contain credentials in auth flows)
