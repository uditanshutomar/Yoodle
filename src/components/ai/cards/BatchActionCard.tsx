  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-2">
        ...
        <button
          onClick={toggleAll}
          className="text-[10px] text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors font-body"
        >
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0">
                <p className="text-[11px] font-medium text-[var(--text-primary)] truncate font-body">
                  {item.title}
                </p>
                {item.subtitle && (
                  <p className="text-[9px] text-[var(--text-muted)] truncate font-body">
                    {item.subtitle}
                  </p>
                )}
              </div>
      ...
      <div className="flex items-center justify-end gap-2 mt-2 pt-2 border-t border-[var(--border)]">
        {isConfirmed ? (
          <span className="flex items-center gap-1.5 text-[11px] font-semibold text-green-500 ml-auto font-heading">
            <Check size={14} /> Confirmed
          </span>
        ) : (
          <>
            <span className="text-[10px] text-[var(--text-muted)] mr-auto font-body">
              {selectedIds.size} of {data.items.length} selected
            </span>
            <button
              onClick={handleConfirm}
              disabled={selectedIds.size === 0 || isConfirming}
              className="flex items-center gap-1 px-3 py-1.5 text-[11px] font-bold rounded-lg bg-[#FFE600] text-[#0A0A0A] border-2 border-[var(--border-strong)] shadow-[2px_2px_0_var(--border-strong)] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all disabled:opacity-40 disabled:shadow-none font-heading"
            >
