...
export default function GhostRoomPage() {
  ...
  if (error || !room) {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h2 className="text-xl font-bold text-[#0A0A0A] mb-2" style={{ fontFamily: "var(--font-heading)" }}>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4 h-full">
      ...
      {/* Back link */}
      <Link href="/ghost-rooms" className="inline-flex items-center gap-1 text-sm text-[#0A0A0A]/50 hover:text-[#7C3AED] transition-colors" style={{ fontFamily: "var(--font-body)" }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4" style={{ height: "calc(100vh - 320px)" }}>
        ...
        <div className="space-y-4">
          ...
          {/* Participants */}
          <div className="bg-white border-2 border-[#7C3AED] rounded-2xl shadow-[4px_4px_0_#7C3AED] p-5">
            <h3 className="text-base font-bold text-[#0A0A0A] mb-3" style={{ fontFamily: "var(--font-heading)" }}>
              Participants ({totalParticipants})
            </h3>
            <div className="space-y-2">
              {room.participants?.map((p) => (
                <div key={p.userId} className="flex items-center justify-between text-sm">
                  <span className="text-[#0A0A0A]" style={{ fontFamily: "var(--font-body)" }}>
          ...
          {/* Room Info */}
          <div className="bg-white border-2 border-[#0A0A0A]/10 rounded-2xl p-4 text-xs text-[#0A0A0A]/50" style={{ fontFamily: "var(--font-body)" }}>
