...
function createRequest(body?: object) {
  return new NextRequest(
    `http://localhost:3000/api/meetings/${TEST_MEETING_ID}/admit`,
    {
      method: "POST",
      headers: {
...
describe("POST /api/meetings/[meetingId]/admit", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockedGetUserId.mockResolvedValue(TEST_USER_ID);
  });

  it("admits a waiting participant", async () => {
    const fakeMeeting = {
    ...
    expect(body.data.admitted).toBe(true);
    expect(mockWaitingGrantAdmission).toHaveBeenCalledWith(
      TEST_MEETING_ID,
      TARGET_USER_ID,
    );
  });
